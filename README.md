# @taktikorg/numquam-esse&nbsp;&nbsp;[![Build Status](https://github.com/taktikorg/numquam-esse/workflows/CI/badge.svg)](https://github.com/taktikorg/numquam-esse/actions)&nbsp;[![Coverage Status](https://coveralls.io/repos/github/pinojs/@taktikorg/numquam-esse/badge.svg?branch=master)](https://coveralls.io/github/pinojs/@taktikorg/numquam-esse?branch=master)
...
```
npm install @taktikorg/numquam-esse -g
```

## Usage

```
  @taktikorg/numquam-esse

  To send pino logs to elasticsearch:

     cat log | @taktikorg/numquam-esse --node http://localhost:9200
...
const pinoElastic = require('@taktikorg/numquam-esse')
...
The stream handler returned from the default factory function is an [`EventEmitter`](https://nodejs.org/api/events.html#events_class_eventemitter).  Developers can use this interface to debug issues encountered by the `@taktikorg/numquam-esse` library.
...
| ----- | ------------------ | ----------- |
| `unknown` | `(line: string, error: string) => void` | Event received by `@taktikorg/numquam-esse` is unparsable (via `JSON.parse`) |
...
```
cat log | @taktikorg/numquam-esse --node https://user:pwd@localhost:9200
```

Alternatively you can supply a combination of `username` and `password` OR `api-key`:
```
cat log | @taktikorg/numquam-esse --node https://localhost:9200 -u user -p pwd
```
```
cat log | @taktikorg/numquam-esse --node https://localhost:9200 --api-key=base64EncodedKey
```

Elastic cloud option `cloud` is also supported:
```sh
cat log | @taktikorg/numquam-esse --cloud=name:bG9jYWxob3N0JGFiY2QkZWZnaA== --api-key=base64EncodedKey
```
...
use @taktikorg/numquam-esse as a module is simple, use [pino-multi-stream](https://www.npmjs.com/package/pino-multi-stream) to send log to multi transport, for example:

```js
const pinoms = require('pino-multi-stream')
const pinoEs = require('@taktikorg/numquam-esse')({
...
Setting up @taktikorg/numquam-esse is easy, and you can use the bundled
...
You can test it by launching `node example | @taktikorg/numquam-esse`, in
this project folder. You will need to have `@taktikorg/numquam-esse`
installed globally.
